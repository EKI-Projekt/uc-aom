...
RUN env VERSION=v0.4.0 GOOS=linux GOARCH=amd64 CGO_ENABLED=0 \
    go build \
...
COPY scripts/docker/v0_4_0/source-credentials.json /tmp/source-credentials.json
COPY scripts/docker/v0_4_0/target-credentials-template.json /tmp/target-credentials-template.json
