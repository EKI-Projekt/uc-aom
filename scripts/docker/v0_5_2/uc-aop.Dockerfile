...
RUN env VERSION=v0.5.2 GOOS=linux GOARCH=amd64 CGO_ENABLED=0 \
    go build \
...
COPY scripts/docker/v0_5_2/source-credentials.json /tmp/source-credentials.json
COPY scripts/docker/v0_5_2/target-credentials-template.json /tmp/target-credentials-template.json
