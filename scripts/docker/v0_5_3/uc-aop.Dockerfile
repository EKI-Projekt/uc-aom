...
# Add checkout of tag
# RUN git checkout -f tags/0.5.3 -b release/0.5.3

WORKDIR /go/src/uc-aom

RUN env VERSION=v0.5.3 GOOS=linux GOARCH=amd64 CGO_ENABLED=0 \
    go build \
...
COPY scripts/docker/v0_5_3/source-credentials.json /tmp/source-credentials.json
COPY scripts/docker/v0_5_3/target-credentials-template.json /tmp/target-credentials-template.json
